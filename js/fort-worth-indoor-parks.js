// Fort Worth Indoor Dog Parks JavaScript
document.addEventListener('DOMContentLoaded', function() {
    // Filter Fort Worth parks from the full dataset
    const fortWorthParks = indoorParksData.filter(park => 
        park.metro_area === 'Fort Worth Metro' || 
        park.city.toLowerCase().includes('fort worth')
    );

    let filteredParks = [...fortWorthParks];
    ...
    window.searchParks = function() {
        ...
        filteredParks = fortWorthParks.filter(park => {
...
function updateResultsCount(count) {
    ...
    if (count === fortWorthParks.length) {
        resultsElement.textContent = `Showing all ${count} indoor dog parks and facilities`;
    } else {
        resultsElement.textContent = `Showing ${count} of ${fortWorthParks.length} indoor dog parks`;
    }
}
